...
設定画面の **テスト表示** で、トーストの見た目だけを試すこともできます。
...
- トーストは通常のウェブページにのみ表示できます。`chrome://` ページや Chrome ウェブストアを開いているときは、バッジとポップアップだけが更新されます。
- Atom フィードは Google が公式にドキュメント化した API ではありません。将来仕様が変わる可能性があります。

## 開発

```
npm test        # フィードの解析と新着通知フローのテスト（node --test）
```

テストは Chrome の API をモックして `service_worker.js` を実際に読み込み、初回の抑制・重複排除・除外キーワード・
件数上限・未ログイン時の扱いまでを検証します。ブラウザは不要です。
